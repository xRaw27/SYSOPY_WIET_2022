...
void (*wc_to_temp)(int, char**);
size_t (*save_block)(int);
void (*delete_block)(size_t);
...
void init_handle() {
    ...
    if (!handle) {
        printf("[init_handler] Error while loading dynamic library\n");
        exit(EXIT_FAILURE);
    }

    create_table = (void (*)(size_t))dlsym(handle, "create_table");
    wc_to_temp = (void (*)(int, char**))dlsym(handle, "wc_to_temp");
    save_block = (size_t (*)(int))dlsym(handle, "save_block");
    delete_block = (void (*)(size_t))dlsym(handle, "delete_block");
    print_table = (void (*)(void))dlsym(handle, "print_table");
    free_table = (void (*)(void))dlsym(handle, "free_table");

    if (dlerror() != NULL) {
        printf("[init_handler] Error while loading functions: %s\n", dlerror());
        exit(EXIT_FAILURE);
    }
}
